...
def check_done(self):
        if self.horizon == 'variable' and self.action_type not in ['ori_only', 'pos_only'] and self.contact_type == 0:
            return self.pos_distance <= self.distance_threshold_pos and self.angle <= self.distance_threshold_ori and self.isHolding == 1 and self.cappedforce <=self.maxforce #and self.anycontact == 0
            ...
            return self.angle <= self.distance_threshold_ori and self.isHolding == 1 and self.cappedforce <=self.maxforce
        elif self.action_type == 'pos_only':
            return self.pos_distance <= self.distance_threshold_pos and self.isHolding == 1 and self.cappedforce <=self.maxforce
        else:
            return self.pos_distance <= self.distance_threshold_pos and self.angle <= self.distance_threshold_ori and self.isHolding == 1 and self.cappedforce <=self.maxforce and self.anycontact == 0