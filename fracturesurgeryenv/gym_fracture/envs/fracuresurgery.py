...
class fracturesurgery_env(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.objectUid = p.loadURDF(foot_path, basePosition=fracturestart, 
                                    baseOrientation=footorientation,
                                     globalScaling=1)
        p.changeDynamics(self.objectUid, -1, mass=0.1, lateralFriction=0.5)
        ...
        env_utils.set_observation(self, initialpos, initialor, 
                                               ...
                                               initialJointVelocities, left_contact,
    ...
    def step(self, action):
        ...
        #     #p.setJointMotorControlArray(self.pandaUid, list(range(9)), p.POSITION_CONTROL, list(step), forces=max_force, maxVelocities=max_vel)
        ...
        joint_torques = p.calculateInverseDynamics(self.pandaUid, jointPoses, max_vel, [1.0]*9)
        ...
        #print(f"Joint Torques Step: {forces}")
            #time.sleep(1./500)  # Remove for speed
        ...
        force = utils.visualize_contact_forces(self.pandaUid, self.objectUid, scale=0.01, lifeTime=5)
