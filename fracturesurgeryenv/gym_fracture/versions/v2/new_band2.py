...
class ElasticBand:
    def __init__(self, bodyA, linkA, bodyB, linkB,
        ...
        # Create ligaments positioned around the bone at different angles
        if num_ligaments == 1:
            ligament_angles = [0.0]
        else:
            ligament_angles = np.linspace(0, 2*np.pi, num_ligaments, endpoint=False)
        
        for angle in ligament_angles:
            # Rotate base offsets around the z-axis (bone's longitudinal axis)
            ...
            rotated_offset_A = rotation_matrix @ base_offset_A
            rotated_offset_B = rotation_matrix @ base_offset_B
            
            # Add randomization to position if enabled
            if randomize_position:
                rotated_offset_A = rotated_offset_A + np.random.normal(0, position_std, 3)
                rotated_offset_B = rotated_offset_B + np.random.normal(0, position_std, 3)
            
            ## Create evenly spaced attachment points centered on the rotated offset.
            ...
            for offset in offsets:
                ...
                self.local_offsets_A.append(rotated_offset_A + delta)
                self.local_offsets_B.append(rotated_offset_B + delta)
        posA = np.array(posA)
        posB = np.array(posB)
        velA = np.array(velA)
        velB = np.array(velB)
